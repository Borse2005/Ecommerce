            ...
            @forelse ($product as $products)
                <a href="{{ route('dash.show', $products->category_id) }}" class="group">
                    ...
                    <h3 class="mt-4 text-sm text-gray-700">
                        {{ Str::substr($products->product, 0, 42) }}
                    </h3>
                    <span class="mt-1 text-lg font-medium text-gray-900">
                        ₹{{ number_format($products->price - $products->discount) }}
                    </span>
                    <span class="text-green-700 ml-3 font-bold">
                        {{ ceil(($products->discount / $products->price) * 100) }}% off
                    </span>
                </a>
            @empty
                <div>
...
<div class="bg-white">
    <div
        class="max-w-2xl mx-auto py-12 px-4 grid items-center grid-cols-1 gap-y-16 gap-x-8 sm:px-6 sm:py-16 lg:max-w-7xl lg:px-8 lg:grid-cols-2">
        <div>
